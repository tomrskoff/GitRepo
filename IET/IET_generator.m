function [facevec1, facevec2, facevec3, tasktype, stimperblock, onsetLoc, offsetLoc] = IET_generator(blockDuration, eventDuration, ISI, IBI, totalDur, d, facenumber,Race_Ratio)
...
for i = 1:numBlocks
    ...
    for k = 1:IBI_numEvents(i)
        [facevec1{count}, facevec2{count}, facevec3{count}, faces] = IET_blurredFaces(blurred_previous, racekey(IBI_count)); %#ok
    ...
    for j = 1:stimperblock
        if tasktype(i) == 1
            [facevec1{count}, facevec2{count}, facevec3{count}] = IET_condition1(d, facenumber, sex(j), race(j), face_previous);
        elseif tasktype(i) == 2
            [facevec1{count}, facevec2{count}, facevec3{count}] = IET_condition2(d, facenumber, sex(j), race(j), face_previous);
        elseif tasktype(i) == 3
            [facevec1{count}, facevec2{count}, facevec3{count}] = IET_condition3(d, facenumber, sex(j),race(j), face_previous);
        end
