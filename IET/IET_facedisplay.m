function answerSide = IET_facedisplay(facecue, facecorrect, faceincorrect, window, screenXpixels, screenYpixels,yCenter, white)

if facecue(9)=='B' % Goes to blurred directory
    image1 = imread(strcat(fileparts(which('IET.m')), filesep, 'Blurred_Faces', filesep, facecue));
    image2 = imread(strcat(fileparts(which('IET.m')), filesep, 'Blurred_Faces', filesep, facecorrect));
    image3 = imread(strcat(fileparts(which('IET.m')), filesep, 'Blurred_Faces', filesep, faceincorrect));
    Screen('TextSize', window, 70);
    Screen('TextFont', window, 'Times');
    DrawFormattedText(window, 'Match', 'center', yCenter*1.80, white);
else % Goes to no blurred directory
    image1 = imread(strcat(fileparts(which('IET.m')), filesep, 'Face_Bank', filesep, facecue));
    image2 = imread(strcat(fileparts(which('IET.m')), filesep, 'Face_Bank', filesep, facecorrect));
    image3 = imread(strcat(fileparts(which('IET.m')), filesep, 'Face_Bank', filesep, faceincorrect));
    Screen('TextSize', window, 70);
    Screen('TextFont', window, 'Times');
    DrawFormattedText(window, 'Match Face', 'center', yCenter*1.80, white);
end
