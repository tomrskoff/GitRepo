function IET(subID, blockDuration, eventDuration, ISI, IBI, totalDur, eqorcar,Race_Ratio)
%% -------- DESCRIPTION --------
% Master function for the implicit emotion task (IET). 
...
% Condition 2 - cue and correct answer are same faces with different valence
% Condition 3 - cue and correct answer are same faces with difference
% valence, however there is another face that is the same face as the cue
% but different emotion (i.e., conflicting).
% The participant is instructed to match the face by clicking the left
% or right arrowkey.
...
if nargin < 8
    [subID, blockDuration, eventDuration, ISI, IBI, totalDur, eqorcar,Race_Ratio] = IET_userinput;
end
WaitSecs(0.5);
[~, ~, ~,white, window, ~, screenXpixels, screenYpixels, ~, yCenter] = IET_waittostart(eqorcar);

% Get face bank directory
face_dir = dir([fileparts(which('IET.m')) filesep 'Face_Bank' filesep '*.jpg']);
face_size = length(face_dir);

% Generate faces
[facecue, facecorrect, faceincorrect, tasktype, stimperblock, onsetLoc, offsetLoc] = IET_generator(blockDuration, eventDuration, ISI, IBI, totalDur, face_dir, face_size, Race_Ratio); %#ok

% Display faces
count = 1;
tic % Starting time
for i = 1:length(facecue)
    answerSide = IET_facedisplay(facecue{i}, facecorrect{i}, faceincorrect{i}, window, screenXpixels, screenYpixels,yCenter, white);
    if sum(i == onsetLoc) == 1 % Faces corresponding to start of block
        onsetTime(count) = toc %#ok
    end
    [responseTime, response, rightorWrong, eventOffset]= IET_recordResponse(eventDuration, answerSide, screenYpixels, screenXpixels, window, ISI, i, stimperblock);
